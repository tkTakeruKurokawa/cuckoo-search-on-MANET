...
public class IncreaseReplications2 implements Control{
	...
	public IncreaseReplications2(String prefix){
	}


	private static int check(Storage storage, Parameter parameter, Data data){
	...
	// 		int num = check(storage, parameter, data);
	// 		if(num == 1 || num ==3){
	// 			failSetP++;
	// 		}
		...
		for(int dataID=0; dataID<requestList.size(); dataID++){
			...
			if(!upLoaded.get(data.getID())){
				while(true){
					...
					int flag = check(storage, parameter, data);
					if(flag == 0){
						break;
					}
