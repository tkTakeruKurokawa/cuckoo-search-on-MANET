...
set yrange [0:600]
set ylabel 'Num of Replica'
plot "owner_counter.csv" every ::1 using 1:6 with lines title "Owner" lc rgb "blue",\
	"path_counter.csv" every ::1 using 1:6 with lines title "Path" lc rgb "magenta",\
	"relate_counter.csv" every ::1 using 1:6 with lines title "Kageyama" lc rgb "dark-green",\
	"cuckoo_counter.csv" every ::1 using 1:6 with lines title "Cuckoo" lc rgb "red",\
